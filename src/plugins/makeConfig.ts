...
function setConfig(bot: mineflayer.Bot, namespace: string, key: string, value: any, save?: boolean, isPrivate?: boolean) {
  save = save ?? true;
  ...
  if (isPrivate) {
    ...
    if (!fs.existsSync(privatePath)) {
      fs.writeFileSync(privatePath, JSON.stringify({ key: value }, null, 2));
      return;
    }
  ...
  const baseConfigPath = `${bot.configDir}/${namespace}.json`;
  if (!fs.existsSync(baseConfigPath)) {
    fs.writeFileSync(baseConfigPath, JSON.stringify({ key: value }, null, 2));
    return;
  }
  const baseData = JSON.parse(fs.readFileSync(baseConfigPath, 'utf8'));
  baseData[key] = value;
  fs.writeFileSync(baseConfigPath, JSON.stringify(baseData, null, 2));
}
...
export default function inject(bot: mineflayer.Bot) {
  ...
  bot.setConfig = (namespace, key, value, save, isPrivate) => setConfig(bot, namespace, key, value, save, isPrivate);
...
declare module 'mineflayer' {
  interface Bot {
    ...
    loadConfig(namespace: string, defaultData: Record<string, any>): void;
    getConfig(namespace: string, key: string): any | undefined;
    setConfig(namespace: string, key: string, value: any, save?: boolean, isPrivate?: boolean): Record<string, any> | undefined;
  }
}